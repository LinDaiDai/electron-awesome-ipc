...
import { IMainIpc, IBaseIpcProps, EIpcNamespace, TProcessKey, TMessagePort, IIpcMessage, IRequestResponse, IRemovePortMsg, IRenderRegisterMsg, IRenderRegisterReturnValue } from './typings';
import { CHANNEL_RENDER_REGISTER, CHANNEL_RENDER_ADD_PORT, CHANNEL_MESSAGE, CHANNEL_REQUEST, CHANNEL_RENDER_REMOVTE_PORT, CHANNEL_RENDER_PROVIDE_PORT, CHANNEL_CLOSE } from './constants';
...
class MainIpc extends BaseIpc implements IMainIpc {
  ...
  private _addIpcRenderRegisteredListener = (): void => {
    ipcMain.on(CHANNEL_RENDER_REGISTER, (ipcMainEvent: Electron.IpcMainEvent, message: IRenderRegisterMsg) => {
      this.logger.info(CHANNEL_RENDER_REGISTER);
      const { port1, port2 } = new MessageChannelMain();
      const { processKey } = message;
      const registerSuccess = this._registerProcessIpcPort(processKey, ipcMainEvent, port1);
      if (!registerSuccess) {
        return;
      }
  ...
  private _registerProcessIpcPort = (processKey: TProcessKey, ipcMainEvent: Electron.IpcMainEvent, messagePort: TMessagePort): boolean => {
    ...
    if (hasValue) {
      ...
      ipcMainEvent.returnValue = { success: false, message: error } as IRenderRegisterReturnValue;
      return false;
    }
  ...
  private _allProcessAddlistenerProcessMessage = (processKey: TProcessKey, mainPort: Electron.MessagePortMain): void => {
